...
public class Activity_PRNFile extends Activity
{
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data)
  {
    try
    {
      switch(resultCode)
      {
        case PrinterHelper.ACTIVITY_IMAGE_FILE:
          setResult(PrinterHelper.ACTIVITY_IMAGE_FILE, data);
          this.finish();
        case PrinterHelper.ACTIVITY_PRNFILE:
          setResult(PrinterHelper.ACTIVITY_PRNFILE, data);
          this.finish();
      }
    }
    catch(Exception e)
    {
      Log.d("SDKSample", (new StringBuilder("Activity_PRNFile --> onActivityResult ")).append(e.getMessage()).toString());
    }
  }
  ...
  private class Clicker implements OnItemClickListener
  {
    public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,long arg3)
    {
      ...
      else
      {
        ...
        if(file.isFile())
        {
          int ResultType=(Invalid(file)==0?PrinterHelper.ACTIVITY_PRNFILE:PrinterHelper.ACTIVITY_IMAGE_FILE);
